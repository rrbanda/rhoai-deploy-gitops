...
## Step 12: Fix kuadrant-operator OOMKilled (WASM plugin unavailable)

If the gateway returns 503 with `critical envoy wasm: Plugin kuadrant-wasm-shim failed to load`, the WASM binary can't be fetched because the `kuadrant-operator-controller-manager` is OOMKilled.

```bash
# 1. Confirm the operator is crash-looping due to OOM
oc get pods -n rhcl-operator | grep kuadrant-operator-controller
oc get pod -n rhcl-operator $(oc get pods -n rhcl-operator -l app.kubernetes.io/name=kuadrant-operator -o name | head -1 | cut -d/ -f2) -o jsonpath='{.status.containerStatuses[0].lastState.terminated.reason}'
# Expected: OOMKilled

# 2. Check current memory limit (likely 300Mi — too low)
oc get deployment -n rhcl-operator kuadrant-operator-controller-manager \
  -o jsonpath='{.spec.template.spec.containers[0].resources.limits.memory}'

# 3. Patch the CSV directly (OLM will revert deployment patches!)
CSV_NAME=$(oc get csv -n rhcl-operator --no-headers | grep -i rhcl | awk '{print $1}')
echo "CSV: ${CSV_NAME}"

# 4. Find deployment index in the CSV
oc get csv ${CSV_NAME} -n rhcl-operator -o json | python3 -c "
import json,sys
csv=json.load(sys.stdin)
for i,d in enumerate(csv['spec']['install']['spec']['deployments']):
    print(f'{i}: {d[\"name\"]}')"
# Note the index for 'kuadrant-operator-controller-manager' (usually 0)

# 5. Patch CSV memory limit (replace INDEX with number from step 4)
INDEX=0  # <-- change if needed
oc patch csv ${CSV_NAME} -n rhcl-operator --type=json \
  -p "[{\"op\":\"replace\",\"path\":\"/spec/install/spec/deployments/${INDEX}/spec/template/spec/containers/0/resources/limits/memory\",\"value\":\"2Gi\"}]"

# 6. Wait for OLM to reconcile (applies new limits to the deployment)
sleep 15
oc get pods -n rhcl-operator | grep kuadrant-operator-controller
...
> **Why patch CSV instead of deployment?** OLM owns the deployment spec and continuously reconciles it. `oc set resources` changes are reverted within seconds. Patching the CSV is the only permanent fix.
